...
- **Every tile, table row, and chart element must be clickable and show more detail on click** — no exceptions, and every single one must use the SAME interaction pattern (inline expand/collapse within the page). Never use `alert()`, `confirm()`, or any native browser dialog for this — they look broken, block the page, and are inconsistent with everything else on screen. A KPI tile click reveals a short breakdown inline; a pipeline table row click expands an inline detail row with the actual research/notes, not a popup; a chart bar or donut segment shows detail on hover or click. Build this with plain inline JS (toggle a CSS class, no external libraries) every time, not as a stretch feature — and test that it actually works before considering the module done, not just that the code looks right.
